...
/// Shared between the milkman's Analysis tab and the admin's Customers
/// tab - the endpoint differs (milkman hits their own customer route,
/// admin hits the admin-only one) but the calendar UI is identical.
Future<void> showCustomerCalendarPopup(
  BuildContext context, {
  required String entriesEndpoint,
  ...
  return showModalBottomSheet(
    ...
    builder: (context) => _CustomerCalendarSheet(entriesEndpoint: entriesEndpoint, customerName: customerName),
  );
}
...
class _CustomerCalendarSheet extends StatefulWidget {
  ...
  const _CustomerCalendarSheet({required this.entriesEndpoint, required this.customerName});
...
class _CustomerCalendarSheetState extends State<_CustomerCalendarSheet> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await ApiClient.instance.dio.get(widget.entriesEndpoint);
      setState(() {
        _entries = res.data;
        _loading = false;
      });
      ...
      setState(() {
        _loading = false;
        _error = 'Could not load this customer\'s history.';
      });
    }
  }
